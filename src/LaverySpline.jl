...
function computeLaverySpline(pp::PP{3,T,TX}, x::Vector{TX}, z::Vector{T}) where {T,TX}
...
function newQuinticLaverySpline(x::Vector{TX}, z::Vector{T}, eps1 = 1e-3, eps2 = 1e-3, order = 3) where {T,TX}
    h = x[2:end] - x[1:end-1] #h1 = x2-x1
    dz = (z[2:end] - z[1:end-1]) ./ h
    size = length(z)
    nIntervals = 10
