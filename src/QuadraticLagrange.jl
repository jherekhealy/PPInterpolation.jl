...
export QuadraticLagrangeInterpolation

struct QuadraticLagrangeInterpolation{TX, T}
    x::Vector{TX}
    y::Vector{T}
end

Base.length(p::QuadraticLagrangeInterpolation) = Base.length(p.x)
Base.size(p::QuadraticLagrangeInterpolation) = Base.size(p.x)
Base.broadcastable(p::QuadraticLagrangeInterpolation) = Ref(p)


function evaluateMid!(v::AbstractArray{T}, pp::QuadraticLagrangeInterpolation{TX,T}, za::AbstractArray{T}) where {TX,T}
    evaluateMid!(v, pp.x, pp.y, za)
end

function evaluateMid(pp::QuadraticLagrangeInterpolation{TX,T}, z::AbstractArray{T}) where {TX,T}
    v = Array{T}(undef, length(z))
    evaluateMid!(v, pp, pp.x, pp.y, z)
end

function evaluate(pp::QuadraticLagrangeInterpolation{TX,T}, z::T) where {TX,T}
    ppIndex = findIndex(pp, z)
    evaluate(pp,ppIndex,z)
end

@inline function findIndex(pp::QuadraticLagrangeInterpolation{TX,T}, z::T) where {TX,T}
    ppIndex = searchsortedlast(pp.x,z)
    ppIndex = min(max(ppIndex, 2), length(pp.x) - 1)
    ppIndex
end

@inline function evaluate(pp::QuadraticLagrangeInterpolation{TX,T}, ppIndex::Int, z::T) where {TX,T}
